	...
	function returnHtml(s){  
		if(s){
               if(spe == 1){
					...
					if(s[0].length < 3){
						return '<span>'+s[0].slice(0,1)+s[0].slice(-1)+"</span><b>天</b>"+'<span>'+s[1].slice(0,1)+s[1].slice(-1)+'</span><b>小时</b><span >'+s[2].slice(0,1)+s[2].slice(-1)+'</span><b>分</b><span>'+s[3].slice(0,1)+s[3].slice(-1)+'</span><b>秒</b>';
					}else{
						return '<span>'+s[0]+"</span><b>天</b>"+'<span>'+s[1].slice(0,1)+s[1].slice(-1)+'</span><b>小时</b><span >'+s[2].slice(0,1)+s[2].slice(-1)+'</span><b>分</b><span>'+s[3].slice(0,1)+s[3].slice(-1)+'</span><b>秒</b>';		
					}
			 }
	    }
	    else{
